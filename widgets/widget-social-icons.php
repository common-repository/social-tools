...
class ADSWST_Social_Icons_Widget extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct( 'adswst_social_icons_widget', __( 'Social Tools Social Icons', 'adswst' ), [
	...
	public function widget( $args, $instance ) {
		...
		$padding                     = empty( $instance[ 'padding' ] ) ? '' : 'padding:' .  $instance[ 'padding' ] . 'px;';
		...
		if( ! empty( $soc_icons ) ){
			foreach ( $soc_icons as $key => $val ){
			    if( $val != '') {
				    echo '<a href="' . esc_url( $val ) . '" target="_blank" rel="nofollow"><i class="social-icon-' . $key . '"></i></a>';
			    }
			}
		}
		echo '</div>';
